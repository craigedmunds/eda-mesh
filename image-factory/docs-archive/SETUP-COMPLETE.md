...
- ✅ `rebuild-trigger-backstage` stage created (watches node-22-bookworm-slim)
- ✅ `rebuild-trigger-uv` stage created (watches python-3.12-slim)
- ✅ HTTP steps configured to call GitHub API
- ✅ Using `github-workflow-token` secret (managed by Kyverno)
- ✅ Successfully tested - workflows triggered automatically!
