...
### Central Tests
- Location: `backstage/app/tests/acceptance/*.spec.ts`
- Purpose: Core Backstage functionality tests
- Maintained by: Platform team

### Plugin Tests  
- Location: `backstage/app/plugins/*/tests/acceptance/*.spec.ts`
...
1. **Create the test directory structure**:
   ```
   backstage/app/plugins/your-plugin/
   └── tests/
       └── acceptance/
           └── your-plugin.spec.ts
   ```
...
4. **Test locally**:
   ```bash
   cd backstage/app/tests/acceptance
   npm install
   npm run test
   ```
...
### Test Filtering (from backstage/kustomize/kargo)
```bash
