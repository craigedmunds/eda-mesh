...
class KargoE2ETest {
  constructor() {
    this.namespace = 'backstage-kargo';
    this.stageName = 'local';
    this.warehouseName = 'backstage';
    this.backstageNamespace = 'backstage';
  ...
  async run() {
    ...
    try {
      ...
      console.error('❌ Backstage Kargo E2E Test FAILED:', error.message);
      process.exit(1);
    }
  }

  async validateInitialSetup() {
    console.log('📋 Validating initial Kargo setup...');
    
    // Check project exists and is ready
    const project = await this.kubectl(`get project backstage-kargo -n ${this.namespace} -o json`);
    if (!project.status?.conditions?.some(c => c.type === 'Ready' && c.status === 'True')) {
      throw new Error('Kargo project is not ready');
    }
    
    // Check warehouse exists
    const warehouse = await this.kubectl(`get warehouse ${this.warehouseName} -n ${this.namespace} -o json`);
    if (!warehouse.metadata.name) {
      throw new Error('Warehouse does not exist');
    }
    
    // Check stage exists
    const stage = await this.kubectl(`get stage ${this.stageName} -n ${this.namespace} -o json`);
  ...
  async ensureFreightExists() {
    console.log('📦 Ensuring freight exists...');
    
    // Check if freight already exists
    try {
      const freightList = await this.kubectl(`get freight -n ${this.namespace} -o json`);
  ...
  async createPromotion(freightName) {
      ...
      spec: {
        ...
        steps: [
          ...
          },
          {
            ...
            config: {
              path: './repo',
              message: 'Update backstage image to ${{ imageFrom("ghcr.io/craigedmunds/backstage").Tag }} - Automated promotion by Kargo E2E test'
            }
          },
          {
            uses: 'git-push',
    ...
    const promotion = await this.kubectl(`get promotion ${promotionName} -n ${this.namespace} -o json`);
    console.log(`✅ Promotion created: ${promotion.metadata.name}`);
    
    return promotion;
  }

  async waitForPromotionCompletion(promotionName) {
    ...
    while (Date.now() - startTime < this.timeout) {
      try {
        ...
        console.log(`⚠️  Error checking promotion status: ${error.message}`);
      }
  ...
  async validateArgoCDSync() {
    console.log('🔄 Validating ArgoCD sync...');
    
    const startTime = Date.now();
    while (Date.now() - startTime < this.timeout) {
      try {
        const app = await this.kubectl(`get application backstage -n argocd -o json`);
        ...
        console.log(`⚠️  Error checking ArgoCD status: ${error.message}`);
      }
  ...
  async validateBackstageDeployment() {
    ...
    while (Date.now() - startTime < this.timeout) {
      try {
        const deployment = await this.kubectl(`get deployment backstage -n ${this.backstageNamespace} -o json`);
    ...
    while (Date.now() - testStartTime < 60000) { // 1 minute timeout for HTTP test
      try {
        const result = execSync(`kubectl run curl-test-${Date.now()} --image=curlimages/curl --rm -i --restart=Never -- curl -f -s http://backstage.${this.backstageNamespace}.svc.cluster.local:7007/`, {
          encoding: 'utf8',
          stdio: ['pipe', 'pipe', 'pipe']
        });
        
        if (result.includes('<!DOCTYPE html>') || result.includes('Backstage')) {
